...
public class ttt {
    ...
    public static void solvePuzzle(int[][] initial, int[][] goal) {
        PriorityQueue<State> openList = new PriorityQueue<>(Comparator.comparingInt((State a) -> a.cost).thenComparingInt(a -> a.moves));
            ...
            if (isGoalState(current.board, goal)) {
                ...
                while (stop == 0) {
                    ...
                    for (State s : visited) {
                        if (s.name == input)
                        printBoard(s.board);
                        System.out.println("Solution found in " + s.moves + " moves." + "f: " + s.cost );
                    }
