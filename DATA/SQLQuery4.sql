...
--T?NG NGÀY M??N CHO PHI?U M??N CÓ NGÀY N??N SAU 31/10/2023
...
FROM PHIEUMUON
--PH?N 3
-- LI?U KÊ CÁC TH? LO?I SÁCH
SELECT THELOAI.TENTHELOAI 
FROM THELOAI

--LI?T KÊ CÁC B?N ??C CH? ? HÀ N?I
SELECT * FROM BANDOC
WHERE BANDOC.DIACHI LIKE '%HA NOI'

--LI?T KÊ CÁC SÁCH ???C M??N MÀ CH?A TR?
...
--TÌM S? L??NG SÁCH CH?A ???C M??N ÍT NH?T 1 L?N
SELECT COUNT(*) AS SOLUONGSACHCHUAMUON
FROM SACH 
LEFT JOIN PHIEUMUON  ON SACH.MASACH = PHIEUMUON.MASACH
WHERE  PHIEUMUON.MASACH IS NULL

--TÌM S ? L??NG SÁCH ???C M??N ÍT NH?T 1 L?N
SELECT COUNT(*) AS SOLUONG
FROM 
(	
	SELECT COUNT(SACH.MASACH) AS SOLUONGMUON FROM SACH
	LEFT JOIN PHIEUMUON  ON SACH.MASACH = PHIEUMUON.MASACH
	GROUP BY SACH.TENSACH
	HAVING  COUNT(SACH.MASACH) >1
) AS SACHMUONNHIEULAN
