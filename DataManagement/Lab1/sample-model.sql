USE master
go
--Non-Unicode (char, varchar): Lưu trữ dữ liệu theo mã hóa mặc định của SQL Server, thường là single-byte (1 byte/ký tự).
--Unicode (nchar, nvarchar): Lưu trữ dữ liệu theo chuẩn UTF-16, hỗ trợ nhiều ngôn ngữ và ký tự đặc biệt (double-byte: 2 byte/ký tự)--
--Thuộc tính name trong sys.databases (nvarchar) lưu dữ liệu dưới dạng Unicode trong khi đó Northwind là  non-Unicode string--
--Việc dùng N'...' nhằm hỗ trợ chuyển đổi non-Unicode string sang Unicode -
IF  EXISTS (SELECT * FROM sys.databases WHERE name = N'Northwind')
BEGIN
...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F') -- là một inner join sys.sysreferences và sys.sysobjects -- 
